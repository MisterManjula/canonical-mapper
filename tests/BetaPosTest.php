...
use CanonicalMapper\Mapper;
use CanonicalMapper\Output\CanonicalJsonWriter;
use CanonicalMapper\Resolution\Flag;
use CanonicalMapper\Resolution\FlagReason;
use CanonicalMapper\Source\Beta\BetaPosAdapter;
use CanonicalMapper\Source\Beta\VatCode;
...
final class BetaPosTest extends TestCase
{
    ...
    {
        $result = (new Mapper())->run(new BetaPosAdapter(), self::fixture('equivalence/beta.xml'));
    ...
    {
        $result = (new Mapper())->run(new BetaPosAdapter(), <<<'XML'
    ...
    public function testAMalformedNetPriceIsRefusedEvenOnAnItemThatWouldBeWithheldAnyway(): void
    {
        ...
        (new Mapper())->run(new BetaPosAdapter(), <<<'XML'
            <?xml version="1.0" encoding="UTF-8"?>
            <catalogue>
    ...
    public function testAnExportThatIsNotXmlIsRefused(): void
    {
        ...
        (new Mapper())->run(new BetaPosAdapter(), '{"menu": {"products": []}}');
    }

    private static function fixture(string $path): string
    {
        $contents = file_get_contents(dirname(__DIR__) . '/fixtures/' . $path);
